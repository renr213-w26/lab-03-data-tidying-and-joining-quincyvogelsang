...
a.  `glimpse()` at the `country_inflation` data frame and answer the following questions based on the output. How many rows does `country_inflation` have and what does each row represent? How many columns does `country_inflation` have and what does each column represent?
    ...
    The dataset country_inflation has 44 rows showing the different countries. There are 30 columns showing the different years.
...
Even the lowest of the three highest inflation rates (Brazil) is almost 2x as much. With the highest inflation rate of Argentina being significantly higher than the United states.
...
New zealand has the biggest inflation change, which is an increase.
...
```{r}
inflation_pivot <- country_inflation |>
  pivot_longer(
    cols = -country,
    names_to = "year",
...
a.  What is the highest inflation rate observed between 1993 and 2021? The output of the pipeline should be a data frame with one row and three columns. In addition to code and output, your response should include a single sentence stating the country and year.
    ...
    The highest inflation rate observed was in Brazil in 1994.
...
b.  What is the lowest inflation rate observed between 1993 and 2021? The output of the pipeline should be a data frame with one row and three columns. In addition to code and output, your response should include a single sentence stating the country and year.
    ...
    The lowest inflation rate was observed in Ireland in 2009.
...
```{r}
countries_of_interest <-c("Netherlands", "Canada", "Australia", "Japan", "Chile")
```

I chose the following countries to include one country of each continent.
...
```{r}
inflation_subset <- inflation_pivot |>
...
```{r}
ggplot(inflation_subset, aes(x = year, y = annual_inflation, color = country, shape = country)) +
  geom_line()+
  geom_point()+
  labs(
    title = "Annual inflation rates (1993-2021)",
    subtitle = "Comparison of 5 different countries from 5 continents",
    x = "Year",
    y = "Inflation rate (%)",
    Color = "Country",
    Shape = "Country"
...
All countries have quite similar lines, we see a shootout from Chile before the 2000s. But all are currently quite similar which I would not have expected. Japan is the lowest which seems the most logical for me.
...
The data set has 132 rows and 4 columns with the variables country, cpi_division, year and annual_inflation.
...
The data set has 12 rows and 2 columns with the variables id and description.
...
The data set now has 132 rows, 5 columns and the variables are country, cpi_division_id, year, annual_inflation and description.
...
```{r}
divisions_of_interest <- c("Health", "Transport", "Food and non-Alcoholic beverages", "Education", "Clothing and footwear")
```

I chose these 5 to get a broad view of different divisions. They represent essential household costs.
...
```{r}
us_inflation_subset <- us_inflation_joined |>
...
```{r}
ggplot(us_inflation_subset, aes(x = year, y = annual_inflation, color = description, shape = description)) +
  geom_point()+
  geom_line()+
  labs(
    title = "US inflation per category (annually)",
    subtitle = "For selected cpi divisions",
    x = "Year",
...
We see that transport is the one that differs the most over the different years, with the other categories remaining more stable. We also see a significant jump for some divisions after 2021. Probably due to being in the post-pandemic time frame.
